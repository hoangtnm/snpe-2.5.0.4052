...
class OpRow (object):
    def __init__(self, op, prev_rows):
        self.op = op
        ...
        def tensor_name_and_type(t):
            return t.name() + ' (' + t.data_type_string() + ')'
        ...
        def add_encoding_parms(name, encoding):
            def make_encoding_tuple(enc):
                def make_encoding_tuple_(encinfo):
                    return encinfo.bw, encinfo.min, encinfo.max, encinfo.scale, encinfo.offset

                if enc.type == ir_graph.QNN_QUANTIZATION_ENCODING_SCALE_OFFSET:
                    return make_encoding_tuple_(enc.encInfo)
                elif enc.type == ir_graph.QNN_QUANTIZATION_ENCODING_AXIS_SCALE_OFFSET:
                    return make_encoding_tuple_(enc.axisEncInfo.encInfos[0])
                return None

            encoding_tuple = make_encoding_tuple(encoding)
            if encoding_tuple:
                key = name + " encoding"
                encinfo_str = scale_offset_format_func(encoding_tuple)
                self.add_parm(key, ' ' + encinfo_str)
                self.encoding_infos[name] = encinfo_str
                if encoding.type == ir_graph.QNN_QUANTIZATION_ENCODING_AXIS_SCALE_OFFSET:
        ...
        for cache_record_name, cache_meta_info in cache_records.items():
            ...
            if cache_meta_info['compatibility']:
                for j in range(0, cache_meta_info['num_of_subnets']):
                    ...
                    print_row(prep_subnet_info_row(subnet_name + ":"), col_sizes, csv_content)
                    ...
                    if "start_layer_Id" in cache_meta_info[subnet_name].keys():
                        ...
                        print_row(prep_subnet_info_row("  " + subnet_col), col_sizes, csv_content)
                    ...
                    for subnet_key, subnet_value in cache_meta_info[subnet_name].items():
                        ...
                        if isinstance(subnet_value, list):
                            ...
                            for value in subnet_value:
                                ...
                                print_row(prep_subnet_info_row(value), col_sizes, csv_content)
                        else:
    ...
    def get_backend_cache_records(self):
        ...
        for record_idx in range(0, self.cache_reader.record_count):
            ...
            record_dict = {
                ...
                'compatibility': record.is_compatibile
            }
