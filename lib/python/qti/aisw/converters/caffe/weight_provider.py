...
class BlobWeightProvider(object):
    ...
    def get_batch_norm_weights(self, layer, input_channel_prev):
        # Mainline BatchNorm:
        #
        ...
        # input_channel_prev is not used. It's only in place to stay in sync
    ...
    def get_fc_weights(self, layer, input_channel, bias_term):
        if bias_term:
            c_bias = npUtils.blob2arr(self.weights_map[layer.name][-1])
    ...
    def get_scale_weights(self, layer, bias_term, input_channel):
        # input_channel ignored when bias provided
        # Weights are not present or passed when scale layer has 2 inputs
        c_weights = None
        c_bias = numpy.require([0] * input_channel, dtype=numpy.float32)
...
class RandomWeightProvider(object):
    ...
    def get_bn_weights(self, layer):
        ...
        input_channel = buf.get_buf_dims()[1]
        c_weights = self.make_weights((input_channel,))
    ...
    def get_batch_norm_weights(self, layer, input_channel_prev):
        ...
        # if it is being folded in. In this case the prev input channel
        # is provided as a parameter
        if input_channel_prev is None:
            input_layer_name = self._graph.naming_policy.get_caffe_name_mapping(layer.bottom[0])
            buf = self._graph.get_buffer(input_layer_name)
            input_channel = buf.get_buf_dims()[1]
        else:
            input_channel = input_channel_prev
        c_weights = self.make_weights((input_channel,))
        return c_weights, c_weights  # use weights for bias
    ...
    def get_conv_weights(self, layer, bias_term):
        ...
        output_channel = conv_param.num_output
        groups = conv_param.group
        input_layer_name = self._graph.naming_policy.get_caffe_name_mapping(layer.bottom[0])
        buf = self._graph.get_buffer(input_layer_name)
        input_channel = buf.get_buf_dims()[1]
        weights_shape = (output_channel, input_channel//groups, ky, kx)

        macs_per_output = ky*ky*input_channel//groups
        c_weights = self.make_weights(weights_shape, macs_per_output)
        c_bias = self.make_weights((output_channel,))
        return c_weights, c_bias
    ...
    def get_deconv_weights(self, layer, bias_term):
        ...
        buf = self._graph.get_buffer(input_layer_name)
        input_channel = buf.get_buf_dims()[1]
        weights_shape = (input_channel, output_channel//groups, ky, kx)

        macs_per_output = ky*kx*input_channel//groups
        c_weights = self.make_weights(weights_shape, macs_per_output)
        c_bias = self.make_weights((output_channel,))
        return c_weights, c_bias
    ...
    def get_fc_weights(self, layer, input_channel, bias_term):
        ...
        output_channel = fc_parm.num_output
        ...
        for name in layer.bottom:
            ...
            w = self.make_weights((input_size, output_channel), input_size)

        if bias_term:
            c_bias = self.make_weights((output_channel,))
        else:
            c_bias = numpy.zeros((output_channel,), dtype=numpy.float32)
        return w, c_bias
    ...
    def get_lstm_weights(self, layer):
        ...
        buf = self._graph.get_buffer(input_layer_name)
        input_channel = buf.get_buf_dims()[-1]
        output_channel = layer.recurrent_param.num_output
        c_x_weights = self.make_weights((output_channel * 4, input_channel), input_channel)
        c_bias = self.make_weights((output_channel * 4,))
        c_h_weights = self.make_weights((output_channel * 4, output_channel), input_channel)
        return c_x_weights, c_bias, c_h_weights
    ...
    def get_prelu_weights(self, layer):
        ...
        buf = self._graph.get_buffer(input_layer_name)
        input_channel = buf.get_buf_dims()[1]
        return numpy.asarray([(2*random.random())-1 for _ in range(input_channel)], dtype=numpy.float32)

    def get_scale_weights(self, layer, bias_term, input_channel):
        # Bias should always be created. Weights are only created when scale takes a
        # single input
        c_bias = self.make_weights((input_channel,))
