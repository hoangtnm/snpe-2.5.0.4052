...
class UdoGenerator:
    ...
    def setup_file_paths(self, ignore_includes=True, force_generation=False, **files):
        ...
        for udo_package in self.udo_packages:
            ...
            if "CPU" in udo_package.supported_runtimes:
                ...
                # copy makefiles
                make_dir = os.path.join(cpu_root, 'makefiles')
                self.make_udo_dir(make_dir)
                # copy Application.mk

                shutil.copyfile(os.path.join(SHARE_LOC_PREFIX , 'makefiles',
                                                          'Application.mk'), os.path.join(make_dir, 'Application.mk'))
                ...
                for compiler in ['linux-x86_64', 'qnx-aarch64']:
                    ...
                    if os.path.exists(makefile):
                        shutil.copy2(makefile, make_dir)

                # copy CPU Makefile
                make_file_path = os.path.join(cpu_root,'Makefile')
                src_make_file_path = os.path.join(os.path.join(SHARE_LOC_PREFIX, 'makefiles', 'CPU', 'Makefile'))
                shutil.copy2(src_make_file_path, make_file_path)

            if "GPU" in udo_package.supported_runtimes:
                unwanted_patterns += ['*Cpu*']
                gpu_root = os.path.join(src_dir, 'GPU')
                gpu_src_dir = os.path.join(gpu_root, 'src')
                gpu_include_dir = os.path.join(gpu_root, 'include')
                self.make_udo_dir(os.path.join(gpu_src_dir, 'ops'))
                self.make_udo_dir(gpu_include_dir)

                # copy source files
                shutil.copyfile(os.path.join(SHARE_LOC_PREFIX, 'CustomOp', 'GPU', 'GpuCustomOpPackage.cpp'),
                                                os.path.join(gpu_src_dir, 'GpuCustomOpPackage.cpp'))
                shutil.copyfile(os.path.join(SHARE_LOC_PREFIX, 'CustomOp', 'GPU', 'GpuCustomOpPackage.hpp'),
                                                os.path.join(gpu_include_dir, 'GpuCustomOpPackage.hpp'))

                # copy Application.mk
                make_dir = os.path.join(gpu_root, 'makefiles')
                self.make_udo_dir(make_dir)
                shutil.copyfile(os.path.join(SHARE_LOC_PREFIX , 'makefiles',
                                                'Application.mk'),
                                                    os.path.join(make_dir, 'Application.mk'))

                # copy GPU Makefile
                make_file_path = os.path.join(gpu_root,'Makefile')
    ...
    def implement_packages(self, gen_cmake=False):
        ...
        for udo_package in self.udo_packages:
            ...
            for runtime in udo_package.supported_runtimes:
                if runtime == "CPU":
                    makefile_file_paths.append( (os.path.join(jni_package_root, 'src', 'CPU', 'makefiles',
                                                              'Android.mk')))
                elif runtime == "GPU":
                    makefile_file_paths.append((os.path.join(jni_package_root, 'src', 'GPU', 'makefiles',
                                                              'Android.mk')))
                else:
            ...
            for operator in udo_package.package_info.operators:
                ...
                for i, runtime in enumerate(op_runtimes):
                    if runtime.lower() == 'cpu':
                        impl_file_paths = [
                            os.path.join(jni_package_root, 'src', str(runtime).upper(), 'src', 'ops',
                                         '{}.cpp'.format(op_name)),
                            os.path.join(jni_package_root, 'src', str(runtime).upper(), 'src',
                                         '{}Interface.cpp'.format(udo_package.name))]
                    elif runtime.lower() == 'gpu':
                        impl_file_paths = [
                            os.path.join(jni_package_root, 'src', str(runtime).upper(), 'src', 'ops',
                                         '{}.cpp'.format(op_name)),
                            os.path.join(jni_package_root, 'src', str(runtime).upper(), 'src',
                                         '{}Interface.cpp'.format(udo_package.name)),
                            os.path.join(jni_package_root, 'src', str(runtime).upper(), 'include',
                                         'Operation.hpp')]
                    elif runtime.lower() != 'dsp':
                        impl_file_paths = [
    ...
    def __implement_cmake_package(self, udo_package):
        ...
        for file_path in output_file_paths:
            ...
            path_to_src_files_dict[file_path] = []
            if os.path.isdir(dirname):
                for root, _, files in os.walk(dirname):
                    rel_dir = os.path.relpath(root, dirname)
                    path_to_src_files_dict[file_path].extend([os.path.join(rel_dir, f) for f in files if f.endswith('.cpp')])
            else:
...
class UdoCodeGenerator:
    ...
    def __substitute_makefile_templates(self, udo_package, runtimes, dsp_arch_types):
        ...
        for i, runtime in enumerate(runtimes):
            if runtime.lower() == 'dsp' and dsp_arch_types:
                ...
                udo_package.package_info.backend = "CPU"
                self.render_templates(self._template_files[4],
                ...
                udo_package.package_info.backend = "GPU"
                self.render_templates(self._template_files[4],
                                      self.output_file_path[i],
                                      multiple=False,
                                      package=udo_package,
    ...
    def render_templates(self, template_files, out_files, multiple=True, **args):
        ...
        for i, template_file in enumerate(template_files):
            ...
            if args.get('cpu') or args.get('gpu'):
                udo_package = args.get('package')
                rendered = mytemplate.render(package_info=udo_package.package_info, **args)
            else:
