...
class DLCBackend(QnnConverterBackendBase):
    ...
    def add_tensor(self, orig_node_name, tensor_name, tensor_type, tensor: np.ndarray,
                   ...
                   tensor_data_type=ir_graph.QNN_DATATYPE_FLOAT_32, src_axis_format=None,
                   tensor_axis_format=None, orig_tensor_name=None, params_count=0):
        node_name = self.sanitize_name(orig_node_name)
        data = None
        if is_static:
            data = tensor
    ...
    def get_ir_graph(self, graph):
        ...
        self.model = qnn_modeltools.QnnModel(graph_name,
                                             input_types_map=self._get_c_input_types_dict(graph.inputs_type_dict))
