...
try:
    ...
    print(str(ie))
    print("Please ensure that $QNN_SDK_ROOT/target/x86_64-linux-clang/python/ is in your PYTHONPATH")
    sys.exit(1)
...
class QnnConverterBackendBase(ConverterBackend):
    ...
    def create_tensor_info(self, tensor_name, tensor_type, dims, tensor_data_type=ir_graph.QNN_DATATYPE_FLOAT_32,
        ...
        tensor_name = self._sanitize_tensor_name(tensor_name)
        ...
        tensor_info = {
            "id": qnn_definitions.get_tensor_id_from_name(tensor_name),
            "name": tensor_name,
            "type": tensor_type,
    ...
    def retrieve_tensor_info(self, tensor_name):
        ...
        """
        tensor_name_ = self._sanitize_tensor_name(tensor_name)
        if tensor_name_ not in self._tensors_info:
            raise ValueError("Requested tensor name ({}) not found.".format(tensor_name_))
        return self._tensors_info[tensor_name_]
    ...
    def update_tensors_info(self, tensor_info, new_tensor_name):
        ...
        new_tensor_name_ = self._sanitize_tensor_name(new_tensor_name)
        tensor_info['name'] = new_tensor_name_
        self._tensors_info[new_tensor_name_] = tensor_info
    ...
    def _get_resolved_tensor_info(tensor_info):
        ...
        tensor_info_ = OrderedDict([
            ...
            ("quant_params", quant_params),
            ("current_dims", list(map(int, tensor_info['current_dims']))),
            ("max_dims", list(map(int, tensor_info['max_dims'])))
        ])
    ...
    def mixed_precision_processing(self, graph):
        ...
        for node in graph.list_nodes():
            ...
            for output_name in node.output_names:
                ...
                static_output = output_tensor.is_static()
                if output_tensor is not None and not static_output:
            ...
            for idx, input_name in enumerate(node.input_names):
                ...
                if input_tensor is not None and not static_input:
