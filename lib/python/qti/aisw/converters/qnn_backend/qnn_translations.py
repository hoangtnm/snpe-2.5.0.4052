...
class QnnInputTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        def check_consumer_is_not_cast_to_float(consumer):
            if not (consumer.op.type == op_adapter.CastOp.TRANSLATION_KEY and
        ...
        if node.op.input_type == InputType.OPAQUE or \
                (numpy_dtype_to_qnn.get(dtype_to_np) != ir_graph.QNN_DATATYPE_FLOAT_32 and
                 ...
                 all(map(check_consumer_is_not_cast_to_float, graph.get_op_output_nodes(node)))):
        ...
        if graph.user_custom_io and backend.c_ir_graph is None and node.output_names[0] in graph.quantization_params:
...
class QnnArgMaxArgMinTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        backend.add_node(node.op.name, node.op.arg_type, node.input_names,
                         ...
                         scalar_params={axis_key:
                                        ...
                                        keep_dims_key:
                                            (numpy_dtype_to_qnn[np.dtype('bool')], np.bool(node.op.keep_dims))
                                        })
...
class QnnCastTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        if not graph.keep_int64_inputs and input_op.type == op_adapter.InputOp.TRANSLATION_KEY:
        ...
        if node.op.input_encoding_in == InputEncodings.NV21:
            ...
            scalar_params.update({qnn_definitions.QNN_OP_ARGB_TO_RGB_PARAM_INPUT_ORDER:
                                  ...
                                  qnn_definitions.QNN_OP_ARGB_TO_RGB_PARAM_REVERSE_OUTPUT:
                                      (numpy_dtype_to_qnn[np.dtype('bool')], np.bool(reverse_output))})
...
class QnnConstantTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        backend.add_tensor(node.op.name, node.output_names[0], qnn_definitions.QNN_TENSOR_TYPE_STATIC,
                           node.op.tensor, check_encodings=check_encodings, is_static=True,
                           store_in_bin=store_in_bin, tensor_data_type=numpy_dtype_to_qnn[node.op.tensor.dtype],
                           src_axis_format=src_axis_format, tensor_axis_format=tensor_axis_format,
                           orig_tensor_name="tensor", params_count=node.op.params_count)
...
class QnnConvertTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        if backend.c_ir_graph is not None:
            orig_output_name = list(output_buf.consumers)[0].output_names[0]
            data_type = backend.c_ir_graph.get_output_tensor(orig_output_name).data_type()
            outputs_info = backend.get_outputs_info(node, graph, tensor_data_type=data_type,
                                                    original_output=orig_output_name)
            backend.update_quant_param_info(node, graph, backend, outputs_info[0], recompute_quant_params=False)
        else:
            outputs_info = backend.get_outputs_info(node, graph)
        ...
        backend.add_node(node.op.name, qnn_definitions.QNN_OP_CONVERT, node.input_names, outputs_info,
                         scalar_params={qnn_definitions.QNN_OP_CONVERT_PARAM_DYNAMIC_INPUT_DATA:
                                            (numpy_dtype_to_qnn[np.dtype('bool')],
                                             np.bool(node.op.dynamic_input_data)),
                                        qnn_definitions.QNN_OP_CONVERT_PARAM_DYNAMIC_OUTPUT_DATA:
                                            (numpy_dtype_to_qnn[np.dtype('bool')],
                                             np.bool(node.op.dynamic_output_data))})
...
class QnnCustomOpTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        tensor_params = {}
        scalar_params = {}
        for name, param in node.op.scalar_params.items():
            dtype = self.convert_custom_dtype_to_qnntype(param["data_type"])
            scalar_params[name] = (dtype, param["data"])
        for name, param in node.op.tensor_params.items():
            dtype = self.convert_custom_dtype_to_qnntype(param["data_type"])

            # check dimensions
            if not param['dimensions']:
                raise ValueError("Dimensions for tensor param: {}"
                                 " in node: {} cannot be empty. Got dimensions = []".
                                 format(name, node.op.name, param['dimensions']))

            # store_in_bin param is not relevant to UDO. Hence, if is skipped for UDO
            # store_in_bin param is present in CustomOps but not in UDO
            # tensor_params for SnpeUdo are added in the else
            # TODO: This is a temporary workaround, revisit this later after the CustomIrOp is in place
            if "store_in_bin" in param and param["store_in_bin"]:
                ...
                param_name = backend.create_unique_qnn_tensor_name(node.op.name,
                                                                   name)
                param_info = backend.create_tensor_info(param_name,
                                                        qnn_definitions.QNN_TENSOR_TYPE_STATIC,
                                                        param["dimensions"],
                                                        dtype,
                                                        data=param["data"])
...
class QnnDequantizeTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        if backend.c_ir_graph is not None:
            input_tensor = backend.c_ir_graph.get_output_tensor(input_name)
            if input_tensor is not None and input_tensor.is_quantized():
                input_tensor_encoding = input_tensor.get_encoding()
            elif input_nodes[0].op.type == op_adapter.QuantizeOp.TRANSLATION_KEY:
                input_tensor_encoding = backend.get_producer_encoding(input_nodes[0], graph)
        elif backend.c_ir_graph is None and graph.user_custom_io:
            # Adding this condition to handle the case when custom quantized inputs are given to non quantized model.
            # This prevents the function from throwing the error in else block as in this case the c_ir_graph is None.
            log_info("Applying custom io for a non quantized model with quantized inputs.")
        else:
            raise RuntimeError("Adding Node {} failed. Expected quantized input activation encoding to {} node "
                               "not found.".format(node.op.name, node.op.name))
        ...
        backend.add_node(node.op.name, qnn_definitions.QNN_OP_DETECTION_OUTPUT,
                         ...
                         scalar_params={qnn_definitions.QNN_OP_DETECTION_OUTPUT_PARAM_CONFIDENCE_THRESHOLD:
                                        ...
                                        qnn_definitions.QNN_OP_DETECTION_OUTPUT_PARAM_USE_BG_IN_NMS:
                                            (numpy_dtype_to_qnn[np.dtype('bool')], np.bool(use_bg_in_nms)),
                                        ...
                                        qnn_definitions.QNN_OP_DETECTION_OUTPUT_PARAM_SHARE_LOCATION:
                                            (numpy_dtype_to_qnn[np.dtype('bool')], np.bool(share_location)),
...
class QnnElementwiseTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        backend.add_node(node.op.name, ir_consts_to_qnn[node.op.type],
                         node.input_names,
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        backend.add_node(node.op.name, qnn_definitions.QNN_OP_EXTRACT_GLIMPSE,
                         ...
                         scalar_params={qnn_definitions.QNN_OP_EXTRACT_GLIMPSE_PARAM_CENTERED:
                                            (numpy_dtype_to_qnn[np.dtype('bool')],
                                             np.bool(extract_glimpse_param_centered)),
                                        qnn_definitions.QNN_OP_EXTRACT_GLIMPSE_PARAM_NORMALIZED:
                                            (numpy_dtype_to_qnn[np.dtype('bool')],
                                             np.bool(extract_glimpse_param_normalized)),
...
class QnnExpandTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        backend.add_tensor(const_op_name, const_op_name, qnn_definitions.QNN_TENSOR_TYPE_STATIC,
                           const_op.tensor, check_encodings=check_encodings, is_static=True,
                           store_in_bin=store_in_bin, tensor_data_type=input_tensor_data_type,
                           src_axis_format=src_axis_format, tensor_axis_format=tensor_axis_format,
                           orig_tensor_name="tensor", params_count=node.op.params_count)
...
class QnnLstmTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        direction = qnn_definitions.QNN_OP_LSTM_DIRECTION_FORWARD
        if node.op.backward:
            direction = qnn_definitions.QNN_OP_LSTM_DIRECTION_REVERSE
        ...
        backend.add_node(node.op.name, qnn_definitions.QNN_OP_LSTM, node.input_names,
                         backend.get_outputs_info(node, graph, tensor_data_type=ir_graph.QNN_DATATYPE_FLOAT_32),
                         scalar_params={qnn_definitions.QNN_OP_LSTM_PARAM_DIRECTION:
                                            (numpy_dtype_to_qnn[np.dtype('uint32')], np.uint32(direction)),
                                        qnn_definitions.QNN_OP_LSTM_PARAM_CELL_CLIP_THRESHOLD:
                                            (numpy_dtype_to_qnn[np.dtype('float32')],
                                             np.float32(node.op.cell_clip_threshold)),
                                        qnn_definitions.QNN_OP_LSTM_PARAM_OUTPUT_CLIP_THRESHOLD:
                                            (numpy_dtype_to_qnn[np.dtype('float32')],
                                             np.float32(node.op.output_clip_threshold)),
                                        qnn_definitions.QNN_OP_LSTM_PARAM_INPUT_GATE_QSCALE:
                                            (numpy_dtype_to_qnn[np.dtype('float32')], np.float32(0.0)),
                                        qnn_definitions.QNN_OP_LSTM_PARAM_FORGET_GATE_QSCALE:
                                            (numpy_dtype_to_qnn[np.dtype('float32')], np.float32(0.0)),
                                        qnn_definitions.QNN_OP_LSTM_PARAM_CELL_GATE_QSCALE:
                                            (numpy_dtype_to_qnn[np.dtype('float32')], np.float32(0.0)),
                                        qnn_definitions.QNN_OP_LSTM_PARAM_OUTPUT_GATE_QSCALE:
                                            (numpy_dtype_to_qnn[np.dtype('float32')], np.float32(0.0)),
                                        qnn_definitions.QNN_OP_LSTM_PARAM_HIDDEN_STATE_OFFSET:
                                            (numpy_dtype_to_qnn[np.dtype('float32')], np.float32(0.0)),
                                        qnn_definitions.QNN_OP_LSTM_PARAM_HIDDEN_STATE_QSCALE:
                                            (numpy_dtype_to_qnn[np.dtype('float32')], np.float32(0.0))
                                        },
                         macs=node.op.macs
                         )
...
class QnnNeuronTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        backend.add_node(node.op.name, qnn_op_type,
                         node.input_names,
                         backend.get_outputs_info(node, graph),
...
class QnnPoolTranslation(QnnTranslationBase):
    ...
    def get_pool_params(self, backend, graph, node):
        ...
        if node.op.pool_type == ir_graph.QNN_OP_POOL_AVG_2D:
            ...
            scalar_params.update({ir_graph.QNN_OP_POOL_AVG_2D_PARAM_COUNT_PAD_FOR_EDGES:
                                      (numpy_dtype_to_qnn[np.dtype('bool')],
                                       np.bool(node.op.count_pad_for_edges))
                                  })
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        backend.add_node(node.op.name, node.op.pool_type,
                         node.input_names,
                         backend.get_outputs_info(node, graph,
                                                  tensor_data_type=ir_graph.QNN_DATATYPE_FLOAT_32),
                                                  tensor_params=tensor_params,
...
class QnnReductionTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        ...
        backend.add_node(node.op.name, ir_consts_to_qnn[node.op.type]["qnn_type"],
                         node.input_names,
                         backend.get_outputs_info(node, graph),
...
class QnnTopKTranslation(QnnTranslationBase):
    ...
    def add_op_to_backend(self, node, graph, backend, **kwargs):
        outputs_info = backend.get_outputs_info(node, graph)
        outputs_info[1]["data_type"] = ir_graph.QNN_DATATYPE_INT_32
