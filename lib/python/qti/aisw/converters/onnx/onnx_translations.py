...
class OnnxTranslationBase(translation.ConversionTranslationBase):
    ...
    def fetch_constant_op(name, converter_context, *, prunable=True, quantizable=None, dtype=None, fail_if_dynamic=True,
        ...
        if not graph.has_buffer(name) and converter_context.weights.has(name):
            buf_value = converter_context.weights.fetch(name, prunable=prunable, dtype=dtype)
...
class ElementwiseBinaryTranslationBase(OnnxTranslationBase, metaclass=ABCMeta):
    ...
    def add_op(self, src_op, converter_context):
        ...
        return graph.add_chained_eltwise_ops(op.eltwise_type, str(src_op.name), self.input_names, output_names[0])
    ...
    def extract_parameters(self, src_op, converter_context):
        ...
        if len(const_input_data) == len(self.input_names):
            ...
            data = self.numpy_op(*const_input_data)
        ...
        return op_adapter.ElementwiseBinaryOp(str(src_op.name), eltwise_type=self.eltwise_type)
...
class OnnxInputTranslation(OnnxTranslationBase):
    ...
    def add_input_op(self, input_, graph, **kwargs):
        ...
        if input_.type.tensor_type.elem_type == TensorProto.INT64 and graph.keep_int64_inputs:
        ...
        if input_.type.tensor_type.elem_type == TensorProto.INT64 and graph.keep_int64_inputs:
            graph.add(op_adapter.CastOp(name + "_cast_int32", from_type="int64", to_type="int32"),
                      [name],
                      [name + "_cast_int32"])
            graph.int64_input_cast_map[name] = name + "_cast_int32"
        return in_node
