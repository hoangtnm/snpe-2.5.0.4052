...
class OnnxCastTranslation(OnnxTranslationBase):
    ...
    def extract_parameters(self, src_op, converter_context):
        ...
        if const_op:
            ...
            return op_adapter.ConstantOp(str(src_op.output[0]), const_op.tensor)
        if not from_type:
...
class OnnxDequantizeLinearTranslation(OnnxTranslationBase):
    ...
    def extract_parameters(self, src_op, converter_context):
        ...
        if converter_context.weights.has(inputs[0]):
            ...
            w = converter_context.weights.fetch(inputs[0])
            w = (w - zp) * scale
...
class OnnxGatherTranslation(OnnxTranslationBase):
    ...
    def add_op(self, src_op, converter_context, **kwargs):
        graph = converter_context.ir_graph
        input_op, translated_ops = self.extract_parameters(src_op, converter_context)
        input_names = self.extract_input_names(src_op, converter_context)
        output_names = self.extract_output_names(src_op, converter_context)
    ...
    def extract_parameters(self, src_op, converter_context):
        ...
        if len(const_input_ops) == len(input_names):
            ...
            converter_context.weights.insert(str(src_op.output[0]), gather_data, was_scalar=was_scalar)
        ...
        if converter_context.weights.has(indices_name) and converter_context.weights.was_scalar(indices_name):
...
class OnnxGatherElementsTranslation(OnnxTranslationBase):
    ...
    def _perform_static_gather_elements(self, input_data: np.ndarray, input_indices: np.ndarray, axis: np.int):
        # validate the inputs
...
class OnnxQuantizeLinearTranslation(OnnxTranslationBase):
    ...
    def extract_parameters(self, src_op, converter_context):
        ...
        log_assert(converter_context.weights.has(inputs[1]),
                   code_to_message.get_error_message("ERROR_STATIC_QUANTIZE_PARAM")("scale", src_op.name, src_op.op_type))
        scale = converter_context.weights.fetch(inputs[1], prunable=False)
        ...
        if len(inputs) > 2:
            log_assert(converter_context.weights.has(inputs[2]),
                       code_to_message.get_error_message("ERROR_STATIC_QUANTIZE_PARAM")("zero point", src_op.name, src_op.op_type))
            zp = converter_context.weights.fetch(inputs[2], dtype=converter_context.weights.type(inputs[2]), prunable=False)
...
class OnnxTileTranslation(OnnxTranslationBase):
    ...
    def extract_parameters(self, src_op, converter_context):
        ...
        input_constant_op = self.fetch_constant_op(input_names[0], converter_context, prunable=False, fail_if_dynamic=False)
        if input_constant_op is not None and not graph.has_buffer(input_names[0]):
            graph.add(input_constant_op, [], [input_names[0]])
            graph.add_src_op_info(input_constant_op.name, [], [input_names[0]])
