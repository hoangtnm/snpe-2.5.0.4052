...
class OnnxMatMulTranslation(OnnxTranslationBase):
    ...
    def extract_parameters(self, src_op, converter_context):
        ...
        if converter_context.weights.has(self.input_names[1]):
            ...
            if len(weights.shape) == 2 and weights.shape[0] == input_A_buf.shape[-1]:
                # Add dummy bias for FC
                bias_input_name = weight_input_name + '_b'
                ...
                if not graph.has_buffer(bias_input_name):
                    ...
                    graph.add_src_op_info(bias_node.op.name, [], [bias_input_name])

                # Handle 1D/2D matmul
                if input_A_buf.rank() == 1 or input_A_buf.rank() == 2:
                    self.input_names.append(bias_input_name)
                    return op_adapter.FullyConnectedOp(str(src_op.name))

                # Handle ND matmul
                first_shapes, last_shape = input_A_buf.shape[:-1], input_A_buf.shape[-1]
                # pre-reshape
                pre_reshape_name = self.output_names[0] + '_pre_reshape'
                pre_reshape_node = graph.add(op_adapter.ReshapeOp(pre_reshape_name,
                                                                  shape=[np.prod(first_shapes), last_shape]),
                                             [self.input_names[0]],
                                             [pre_reshape_name])
                graph.add_src_op_info(pre_reshape_node.op.name,
                                      [self.input_names[0]],
                                      [pre_reshape_name])
                # fc
                fc_name = self.output_names[0] + '_fc'
                fc_node = graph.add(op_adapter.FullyConnectedOp(fc_name),
                                    [pre_reshape_name, weight_input_name, bias_input_name],
                                    [fc_name])
                graph.add_src_op_info(fc_node.op.name,
                                      [pre_reshape_name, weight_input_name, bias_input_name],
                                      [fc_name])
                # post-reshape
                post_reshape_name = self.output_names[0] + '_post_reshape'
                post_reshape_node = graph.add(op_adapter.ReshapeOp(post_reshape_name,
                                                                   shape=[*first_shapes, weights.shape[-1]]),
                                              [fc_name],
                                              self.output_names,
                                              axis_formats=[input_A_buf.axis_format])
                graph.add_src_op_info(post_reshape_node.op.name,
                                      [fc_name],
                                      self.output_names)
                return None
