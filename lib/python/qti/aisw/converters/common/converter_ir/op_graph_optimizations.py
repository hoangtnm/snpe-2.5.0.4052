...
supported_opt_list = [SQUASH_SCALE, SQUASH_PROD, SQUASH_DIV, SQUASH_SUM, SQUASH_SUB, SQUASH_RESHAPE, SQUASH_BATCHNORM, FOLD_CONCATS, FOLD_RESHAPES,
...
class IROptimizations(object):
    ...
    def get_custom_io_permute_order(self, initial_axis_format, custom_axis_format):
        permute_order = None
        reverse_order = None
        ...
        for entry in graph.user_custom_io:
            ...
            if "Layout" in entry:
                ...
                if initial_axis_format == custom_axis_format:
                    ...
                    continue
                permute_order, reverse_order = self.get_custom_io_permute_order(initial_axis_format, custom_axis_format)
                log_assert((permute_order is not None and reverse_order is not None),"Invalid custom layout tranformation for buffer {}."
                    .format(buffer_name))

                if isInput:
                    # In case of custom input layout, modify the shape of the input node
                    ...
                    buf.shape = new_shape
                    buf.axis_format = custom_axis_format
                    node.op.shape = buf.shape
                    graph.inject_implicit_permute(buffer_name, initial_axis_format, permute_order, consumers)

                if isOutput:
                    # In case of custom output layout, inject a permute op after the output and replace the original output
                    # with the output of permute op in graph.output_names list
                    graph.inject_implicit_permute(buffer_name, custom_axis_format, reverse_order, [])
            ...
            if "Datatype" in entry:
                ...
                if isOutput:
                    ...
                    graph.inject(cast_op, buffer_name, buffer_name+"_custom_cast", consumer_names=[])
                    new_output_node = list(graph.buffers[buffer_name].consumers)[0]
                    new_output_buffer_name = new_output_node.output_names[0]
                    graph.output_names.remove(buffer_name)
                    graph.output_names.append(new_output_buffer_name)
...
class OptimizeBatchnormTranslation(OptimizationTranslationBase):
    ...
    def squash_batchnorm(graph):
        ...
        def manage_shared_static_input(consumer, idx):
            buffer = graph.get_buffer(consumer.input_names[idx])
            producer_op = buffer.producer.op
            weight_tensor = producer_op.tensor
            weight_tensor_copy = np.copy(weight_tensor)
            if idx == 1:
                name = consumer.op.name + "_kernel_weight"
            else:
                name = consumer.op.name + "_kernel_bias"
            const_op = op_adapter.ConstantOp(name, tensor=weight_tensor_copy)
            producer_idx = graph.list_nodes().index(buffer.producer)
            graph.add(const_op, [], [name], axis_formats=[buffer.axis_format], idx=producer_idx+1)
            graph.get_buffer(name).consumers.add(consumer)
            buffer.consumers.remove(consumer)
            consumer.input_names[idx] = name
            # copy quant overrides to new const buffer only if it exists in original static input
            if(graph.has_quantization_param(buffer.name)):
                quant_param = graph.get_layer_quantization_param(buffer.name)
        ...
        for sequence in sequences:
            for node_tuple in graph.get_matched_nodes(sequence, validator=validate):
                ...
                if len(graph.get_buffer(conv_node.input_names[1]).consumers) > 1 :
                    manage_shared_static_input(conv_node, 1)
                ...
                if len(graph.get_buffer(conv_node.input_names[2]).consumers) > 1 :
                    manage_shared_static_input(conv_node, 2)
    ...
    def axes_to_spatial_first_order(self, node, graph):
        ...
        if input_buf.rank() == 4:
            if input_buf.axis_format == AxisTracker.AxisFormat.NSC and \
                ...
                if weights_buf.axis_format != AxisTracker.AxisFormat.NONTRIVIAL:
...
class OptimizeGatherNDTranslation(OptimizationTranslationBase):
    ...
    def axes_to_spatial_first_order(self, node, graph):
        ...
        if [graph.get_input_axis_formats(node)] == node.op.data_axis_formats:
            return False
    ...
    def match_gelu(graph):
        ...
        for sequence in sequences:
            ...
            for node_tuple in matched_node_list:
                ...
                # For the last_node, two different sequences correspond to two different processes:
                # Sequence1:
...
class OptimizeGruTranslation(OptimizationTranslationBase):
    ...
    def expand_gru_update_gate(self, graph, gru_node, input_x_reshape_node_name, input_h_reshape_node_name):
        ...
        elementsum_of_term1_op = op_adapter.ElementwiseSumOp(name=elementsum_of_term1_op_name)
        ...
        elementsum_of_term1_bias_op = op_adapter.ElementwiseSumOp(name=elementsum_of_term1_bias_op_name)
    ...
    def expand_gru_reset_gate(self, graph, gru_node, input_x_reshape_node_name, input_h_reshape_node_name):
        ...
        elementsum_of_term2_op = op_adapter.ElementwiseSumOp(name=elementsum_of_term2_op_name)
        ...
        elementsum_of_term2_bias_op = op_adapter.ElementwiseSumOp(name=elementsum_of_term2_bias_op_name)
    ...
    def expand_gru_hidden_gate(self, graph, gru_node, input_x_reshape_node_name, input_h_reshape_node_name, rt, linear_before_reset = 0):
        ...
        xt_wh_wbh_matmul_bias_op = op_adapter.ElementwiseSumOp(name=xt_wh_wbh_matmul_bias_op_name)
        ...
        if linear_before_reset == 0:
            ...
            rt_dot_ht_1_op = op_adapter.ElementwiseProductOp(name=rt_dot_ht_1_op_name)
            ...
            rt_dot_ht_1_rh_rbh_matmul_bias_op = op_adapter.ElementwiseSumOp(name=rt_dot_ht_1_rh_rbh_matmul_bias_op_name)
            ...
            elementsum_of_term3p1_op = op_adapter.ElementwiseSumOp(name=elementsum_of_term3p1_op_name)
            ...
            ht_1_rh_rbh_matmul_bias_op = op_adapter.ElementwiseSumOp(name=ht_1_rh_rbh_matmul_bias_op_name)
            ...
            rt_dot_ht_1_rh_rbh_matmul_bias_op_name = gru_node_name + "_rt_dot_ht_1_rh_rbh_matmul_bias_op"
            rt_dot_ht_1_rh_rbh_matmul_bias_op = op_adapter.ElementwiseProductOp(name=rt_dot_ht_1_rh_rbh_matmul_bias_op_name)
            ...
            elementsum_of_term3p2_op = op_adapter.ElementwiseSumOp(name=elementsum_of_term3p2_op_name)
    ...
    def update_gru_hidden_state(self, graph, gru_node, Ht_1, zt, ht):
        ...
        one_minus_zt_op = op_adapter.ElementwiseSubOp(name=one_minus_zt_op_name)
        ...
        one_minus_zt_dot_ht_op = op_adapter.ElementwiseProductOp(name=one_minus_zt_dot_ht_op_name)
        ...
        zt_dot_ht_1_op = op_adapter.ElementwiseProductOp(name=zt_dot_ht_1_op_name)
        ...
        elementsum_of_term4_op = op_adapter.ElementwiseSumOp(name=elementsum_of_term4_op_name)
    ...
    def expand_gru_op_structure(self, graph):
        ...
        for nodes_tuple in matched_node_list:
            ...
            output_all_hiddens_reshape_op = op_adapter.ReshapeOp(name=output_all_hiddens_reshape_node_name,
                                                                 output_shape=output_all_hiddens_output_shape)
            ...
            output_all_hiddens_reshape_op = op_adapter.ReshapeOp(name=output_hidden_reshape_node_name,
                                                                 output_shape=output_hidden_output_shape)
            graph.add(output_all_hiddens_reshape_op, input_names=[Ht],
    ...
    def unroll_lstm_time_steps(graph):
        ...
        for nodes_tuple in matched_node_list:
            ...
            for consumer in list(hidden_output_buffer.consumers):
                output_h_buffer = graph.get_buffer(output_h_name_list[seq_length-1])
                output_h_buffer.consumers.add(consumer)
                h_idx = consumer.input_names.index(lstm_node.output_names[HIDDEN_OUT_IDX])+1
                consumer.input_names.insert(h_idx, output_h_name_list[seq_length-1])
        ...
        for nodes_tuple in matched_node_list:
            ...
            if output_reshape_needed:
                ...
                output_all_reshape_op = op_adapter.ReshapeOp(name=output_all_reshape_node_name,
                                                             shape=output_all_reshape_output_shape)
...
class OptimizeMatmulTranslation(OptimizationTranslationBase):
    ...
    def merge_low_level_ops_to_layers(self, graph):
        def validate_dims(nodes_tuple):
            ...
            add_input_buffer1 = constant_bias_node.op.tensor

            if matmul_node.output_names[0] in elementwise_sum_node.input_names and matmul_input_0_buffer.rank() == 3 \
                    and matmul_input_0_buffer.shape[2] == matmul_input_1_buffer.shape[0] \
                    and matmul_input_1_buffer.rank() == 2 and matmul_input_1_buffer.shape[1] == add_input_buffer1.shape[-1] \
                    and len(list(add_input_buffer1.shape)) == 1:
                return True
            return False
        ...
        for node_tuple in matched_node_list:
            ...
            bias = constant_bias_node.op.tensor
            ...
            post_reshape_name = fc_op.name + "_post_reshape"
            ...
            # Refetch the fc_node to update the output buffer names after squash
            fc_node = graph.get_node_by_name(fc_op.name)
            fc_out_buf = graph.get_output_buffers(fc_node)[0]
            # np.prod assigns default platform integer int64
            # which is not consistent to the default int, dtypes we get are int64 and int
            # converting the shape to numpy array and back to list makes it consistent (int, int)
            fc_out_buf.shape = np.array([np.prod(matmul_input_buffer.shape[:-1]), matmul_weights.shape[-1]]).tolist()

            # inject the post reshape op to the graph
            graph.inject(post_reshape_op, fc_out_buf.name, output_name=post_reshape_name,
...
class OptimizeNeuronTranslation(OptimizationTranslationBase):
    ...
    def merge_low_level_ops_to_layers(self, graph):
        ...
        def validate_node(node_tuple):
            if node_tuple[0].op.TRANSLATION_KEY == "elementwise_min":
        ...
        for idx, sequence in enumerate(sequences):
            ...
            for node_tuple in matched_node_list:
                ...
                _, constant_node_min, tensor_scalar_value_min = self.check_static_equal_tensor_vals_input(min_node, graph)
                _, constant_node_max, tensor_scalar_value_max = self.check_static_equal_tensor_vals_input(max_node, graph)
                ...
                relu_min_max_op.min_value = tensor_scalar_value_min
                relu_min_max_op.max_value = tensor_scalar_value_max
    ...
    def axes_to_spatial_first_order(self, node, graph):
        ...
        if input_buf.axis_format == AxisTracker.AxisFormat.NDHWC:
            if node.op.data_axis_formats[0] == AxisTracker.AxisFormat.NDHWC:
                ...
                node.op.data_axis_formats[0] == AxisTracker.AxisFormat.NCDHW:
            # special case: transforming to NDHWC, will become identity
            if output_buf.axis_format == AxisTracker.AxisFormat.NDHWC and node.op.perm == [0, 2, 3, 4, 1]:
                node.op.perm = [0, 1, 2, 3, 4]
                output_buf.axis_format = AxisTracker.AxisFormat.NDHWC
                return
            ...
            if node.op.data_axis_formats[0] == AxisTracker.AxisFormat.NSC:
                ...
                node.op.data_axis_formats[0] == AxisTracker.AxisFormat.NCS:
            # special case: transforming to NSC, will become identity
            if output_buf.axis_format == AxisTracker.AxisFormat.NSC and node.op.perm == [0, 2, 3, 1]:
                node.op.perm = [0, 1, 2, 3]
                output_buf.axis_format = AxisTracker.AxisFormat.NSC
                return
            ...
            if node.op.data_axis_formats[0] == AxisTracker.AxisFormat.NFC:
                ...
                node.op.data_axis_formats[0] == AxisTracker.AxisFormat.NCF:
            # special case: transforming to NFC, will become identity
            if output_buf.axis_format == AxisTracker.AxisFormat.NFC and node.op.perm == [0, 2, 1]:
                node.op.perm = [0, 1, 2]
                output_buf.axis_format = AxisTracker.AxisFormat.NFC
                return
    ...
    def fold_reshapes(graph):
        ...
        for node_tuple in matched_node_list:
            ...
            if reshape_node_input_buf.producer.op.type == op_adapter.ReshapeOp.TRANSLATION_KEY:
                ...
                # squash previous reshape node into next
                graph.squash(prev_reshape_node, prev_reshape_node_input_names[0], squash_into_next=True)

                log_debug2("Folded Reshape:{} into Reshape:{}".format(prev_reshape_node.op.name, reshape_node.op.name))
    ...
    def merge_low_level_ops_to_layers(self, graph):
        ...
        sequence1 = [
            ...
            (ir_graph.QNN_OP_GATHER,
             ("MATCH_NUM_BUFS", [(ir_graph.QNN_OP_MULTI_CLASS_NMS, "ANY"), ("constant", "ANY")]),
        ...
        sequence3 = [
            ...
            (ir_graph.QNN_OP_STRIDED_SLICE,
             ("MATCH_NUM_BUFS", [(ir_graph.QNN_OP_MULTI_CLASS_NMS, "ALL")]),
             ("FLEXIBLE_NUM_BUFS", [(ir_graph.QNN_OP_GATHER, "ALL")])
            )
        ]
        ...
        for sequence in sequences:
            ...
            for node_tuple in matched_node_list:
                ...
                nms_max_total_detections = onnx_nms_node.op.max_total_detections
...
class OptimizeStridedSliceTranslation(OptimizationTranslationBase):
    ...
    def axes_to_spatial_first_order(self, node, graph):
        ...
        if input_buf.axis_format != AxisTracker.AxisFormat.NONTRIVIAL:
            ...
            begins = graph.src_axis_order.permute_shape_to_ir(begins, input_buf.axis_format)
            ends = graph.src_axis_order.permute_shape_to_ir(ends, input_buf.axis_format)
            strides = graph.src_axis_order.permute_shape_to_ir(strides, input_buf.axis_format)
