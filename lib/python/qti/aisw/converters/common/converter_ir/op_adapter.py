...
class DequantizeOp(Op):
    ...
    def __init__(self, name, **kwargs):
        ...
        attrs.addUint32("bw", kwargs.get("bw"), ir_graph.IrAttrUsageType.IR_ATTR_USAGE_LEGACY)
        attrs.addFloat("min", kwargs.get("min", 0.0), ir_graph.IrAttrUsageType.IR_ATTR_USAGE_LEGACY)
        attrs.addFloat("max", kwargs.get("max", 0.0), ir_graph.IrAttrUsageType.IR_ATTR_USAGE_LEGACY)
        attrs.addFloat("scale", kwargs.get("scale", 0.0), ir_graph.IrAttrUsageType.IR_ATTR_USAGE_LEGACY)
        attrs.addInt32("offset", kwargs.get("offset", 0), ir_graph.IrAttrUsageType.IR_ATTR_USAGE_LEGACY)
        attrs.addBool("is_symmetric", kwargs.get("is_symmetric", False), ir_graph.IrAttrUsageType.IR_ATTR_USAGE_LEGACY)
...
class GatherNDOp(Op):
    TRANSLATION_KEY = 'gather_nd'
    LEGACY_TRANSLATION_KEY = 'gather_nd'

    def __init__(self, name, **kargs):
        Op.__init__(self, name, self.TRANSLATION_KEY)
        self.addattr('batch_dims', kargs, 0)

    def infer_shape(self, input_shapes, input_axis_formats, num_outputs, axis_order):
        r_data_rank=len(input_shapes[0])
        q_indices_rank=len(input_shapes[1])
        b_batch_dims=self.batch_dims

        data_shape=input_shapes[0]
        indices_shape=input_shapes[1]
        output_shape=[]

        if r_data_rank < 1 or q_indices_rank < 1:
            raise ValueError(
                "The data rank and indices rank must be >= 1 for op {}".format(self.name))
        elif data_shape[0:b_batch_dims] != indices_shape[0:b_batch_dims]:
            raise ValueError(
                "The first b dimensions in the shape of indices tensor \
                    and data tensor must be equal for op {}".format(self.name))
        elif b_batch_dims < 0 or b_batch_dims >= min(r_data_rank, q_indices_rank):
            raise ValueError(
                "The batch_dims must be a value in [0, min(data.rank, indices.rank)) for op {}".format(self.name))

        # calculate output shape according to data.shape / indices.shape / batch_dims
        if indices_shape[-1] < 1 or indices_shape[-1] > r_data_rank - b_batch_dims:
            raise ValueError(
                "The indices_shape[-1] must have a value [1, data.rank - batch_dims] for op {}".format(self.name))
        elif indices_shape[-1] == r_data_rank - b_batch_dims:
            output_shape=indices_shape[:b_batch_dims] + indices_shape[b_batch_dims:-1]
        else:
            output_shape=indices_shape[:b_batch_dims] + list(indices_shape)[b_batch_dims:-1] \
                            + list(data_shape)[b_batch_dims + indices_shape[-1]:]
...
class LayerNormOp(Op):
    ...
    LEGACY_TRANSLATION_KEY = 'layer_norm'
    EPSILON = 1e-9
...
class LstmOp(Op):
    ...
    def infer_shape(self, input_shapes, input_axis_formats, num_outputs, axis_order):
        def get_c_h_output_dims(batch_size, output_feature):
            c_t_dims = [batch_size, output_feature]
            h_t_dims = [batch_size, output_feature]
        ...
        if len(input_shape) == 3:
            batch_size, time_steps, _ = axis_order.extract_time_series_dims(input_shape)
        output_feature = self.hidden_size
        output_dims.append(
            axis_order.format_time_series_output_shape(batch_size=batch_size,
                                                       time_steps=time_steps,
        ...
        if self.c_0_input_name and self.h_0_input_name:
            # Layer has exposed recurrent inputs, therefore we need to add c_T and h_T outputs
            c_dims, h_dims = get_c_h_output_dims(batch_size, output_feature)
