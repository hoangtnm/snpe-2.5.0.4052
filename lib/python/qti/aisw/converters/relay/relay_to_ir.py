...
class RelayConverterContext:
    ...
    """
    def __init__(self, quir_graph: IROpGraph, output_names_dict: dict=None, quantization_params_dict: dict=None):
        ...
        self.quantization_params_dict = quantization_params_dict
...
class RelayConverterFrontend(ConverterFrontend):
    ...
    def __init__(self, args, importer: RelayImporter=None, mod: tvm.IRModule=None, params: dict=None, **kwargs):
        ...
        if self.importer and isinstance(self.importer, RelayImporter):
            self.relay_mod, self.relay_params, self.expr_to_out_names_dict, self.expr_to_quantization_params_dict =\
        ...
        self.converter_context = RelayConverterContext(self.graph,
                                                       ...
                                                       quantization_params_dict=self.expr_to_quantization_params_dict)
        self._init_globals()
    ...
    def add_input(expr: relay.expr):
        ...
        if var_name in RELAY_PARAMS:
            ...
            key = get_key_from_expr(expr)
            if CONVERTER_CTX.quantization_params_dict is not None and key in CONVERTER_CTX.quantization_params_dict:
                encodings = CONVERTER_CTX.quantization_params_dict[key]
