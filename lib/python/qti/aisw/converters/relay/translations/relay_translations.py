...
from qti.aisw.converters.common.utils.converter_utils import log_debug, log_debug1, log_debug3
...
class RelayTranslationBase(translation.ConversionTranslationBase, metaclass=ABCMeta):
    ...
    def populate_quantization_params(self, relay_expr: relay.expr, converter_context, quir_graph, names: list, is_param):
        key = hash(relay_expr)
        if converter_context.quantization_params_dict is not None and key in converter_context.quantization_params_dict:
            encodings = converter_context.quantization_params_dict[key]
            # TODO: use dict to get encoding instead of using list
            for encoding, name in zip(encodings, names):
                if isinstance(encoding, list):
                    # per channel quantization
                    quir_graph.set_overridden_encoding(name, encoding, is_param=is_param)
                else:
                    quir_graph.set_overridden_encoding(name, [encoding], is_param=is_param)
