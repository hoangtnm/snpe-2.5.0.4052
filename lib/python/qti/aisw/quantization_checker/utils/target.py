...
class x86_64(target):
    def __init__(self, quantizationVariation, inputList, graphDir, sdkDir, outputDir, configParams):
        super().__init__(quantizationVariation, inputList, graphDir, sdkDir, 'bin/x86_64-linux-clang', outputDir, configParams)

    def buildSnpeNetRunArgs(self):
        self.snpeNetRunCmdAndArgs = os.path.join(self.sdkDir, self.targetBasePath, 'snpe-net-run') + ' --container ' + os.path.join(self.graphDir, self.quantizationVariation, self.unquantizedDlc) + ' --output_dir ' + os.path.join(self.outputDir, self.quantizationVariation) + ' --input_list ' + self.inputList + ' --debug'
