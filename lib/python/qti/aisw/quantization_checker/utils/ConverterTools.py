...
class TensorflowConverter:
    ...
    def convert(self, env, activationWidth=None, biasWidth=None, weightWidth=None, workingDir=None, quantOverrides=None):
        snpeTensorflowConverterBinaryPath = os.path.join(self.__sdkPath, 'bin', 'x86_64-linux-clang', 'snpe-tensorflow-to-dlc')
...
class TfliteConverter:
    ...
    def convert(self, env, activationWidth=None, biasWidth=None, weightWidth=None, workingDir=None, quantOverrides=None):
        snpeTfliteConverterBinaryPath = os.path.join(self.__sdkPath, 'bin', 'x86_64-linux-clang', 'snpe-tflite-to-dlc')
...
class CaffeConverter:
    def __init__(self, logger: Logger, sdkPath, inputNetwork, caffeBin, inputList, quantizationVariations=None, quantizationAlgorithms=None):
        self.__sdkPath = sdkPath
        self.__inputNetwork = inputNetwork
        self.__caffeBin = caffeBin
        self.__inputList = inputList
        self.__snpeCaffeQuantizerArgs = {}
        self.logger = logger
        self.__buildArgs__(quantizationVariations, quantizationAlgorithms)

    def convert(self, env, activationWidth=None, biasWidth=None, weightWidth=None, workingDir=None, quantOverrides=None):
        snpeCaffeConverterBinaryPath = os.path.join(self.__sdkPath, 'bin', 'x86_64-linux-clang', 'snpe-caffe-to-dlc')

        if not workingDir:
            workingDir = os.path.dirname(self.__inputNetwork)
        utils.changeDir(workingDir)

        baseArgs = ' -i ' + self.__inputNetwork + ' --caffe_bin ' + self.__caffeBin
        unquantizedDlc = os.path.join(workingDir, 'unquantized', 'unquantized') + '.dlc'
        outArgs = ' -o ' + unquantizedDlc
        self.logger.print('Converting Caffe model to SNPE DLC', PrintOptions.LOGFILE)
        Progress.updateProgress(Progress.getStepSize(ProgressStage.GENERATOR))

        returnValue = utils.issueCommandAndWait(snpeCaffeConverterBinaryPath + baseArgs + outArgs, self.logger, env)
        if returnValue != 0:
            return ('unquantized', -1)
        Progress.updateProgress(Progress.getStepSize(ProgressStage.GENERATOR))

        resultsMap = {}
        resultsMap = snpeQuantizeDlc(self.__snpeCaffeQuantizerArgs, unquantizedDlc, self.__inputNetwork, self.__sdkPath, env, self.logger, activationWidth, biasWidth, weightWidth, quantOverrides)
        return resultsMap

    def __buildArgs__(self, quantizationVariations=None, quantizationAlgorithms=None):
        self.logger.print('Input list: ' + self.__inputList, PrintOptions.LOGFILE)
        self.__snpeCaffeQuantizerArgs = buildQuantizationParameterMap(quantizationVariations, quantizationAlgorithms, self.__inputList)
...
class OnnxConverter:
    ...
    def convert(self, env, activationWidth=None, biasWidth=None, weightWidth=None, workingDir=None, quantOverrides=None):
        snpeOnnxConverterBinaryPath = os.path.join(self.__sdkPath, 'bin', 'x86_64-linux-clang', 'snpe-onnx-to-dlc')
...
def snpeQuantizeDlc(snpeDlcQuantizeArgs, unquantizedDlc, inputNetwork, sdkPath, env, logger, activationBitwidth=None, biasBitwidth=None, weightBitwidth=None, quantOverrides=None):
    resultsMap = {}
    snpeDlcQuantizeBinaryPath = os.path.join(sdkPath, 'bin', 'x86_64-linux-clang', 'snpe-dlc-quantize')
