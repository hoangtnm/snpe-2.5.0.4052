...
class DataExtractor:
    def __init__(self, quantizationVariations, frameworkModelFile, inputList, outputDir, logger):
        self.quantizationVariations = quantizationVariations
        self.frameworkModelFile = frameworkModelFile
    ...
    def extract(self):
        self.logger.print('Extracting weights and biases from dlc.', PrintOptions.CONSOLE_LOGFILE)
        self.__parseOpsFromDlc()
        self.logger.print('Extracting input file names and input file data.', PrintOptions.CONSOLE_LOGFILE)
    ...
    def __loadSnpeModel(self, dlcFilePath):
        self.logger.print('Loading the following model: ' + dlcFilePath, PrintOptions.CONSOLE_LOGFILE)
        model = ModelInfo()
    ...
    def __extractInputData(self):
        inputDirPath = os.path.dirname(self.frameworkModelFile)
        try:
                    ...
                    for file in filenames.split():
                        if file:
                            ...
                            if not os.path.exists(os.path.join(inputDirPath, file)):
                                self.logger.print('The following file from the input list (' + file + ') could not be found. Exiting...', PrintOptions.CONSOLE_LOGFILE)
                                exit(-1)
                        self.inputFileNames.append(filenames)
                        self.inputData[file] = np.fromfile(os.path.join(inputDirPath, file), dtype=np.float32)
        except Exception as e:
            self.logger.print("Unable to open input list file, please check the file path! Exiting...", PrintOptions.CONSOLE_LOGFILE)
