...
class SqnrCalculation:
    ...
    def __call__(self, unquantizedData, deQuantizedData) -> Dict:
        ...
        if meanSquareError == 0:
            return {'pass': True, 'data': inf, 'threshold': self.threshold}
        else:
            ...
            if logRatio < self.threshold:
                passes = False
            return {'pass': passes, 'data': logRatio, 'threshold': self.threshold}
    ...
    def __call__(self, lhs, rhs) -> Dict:
        ...
        if result:
            ...
            if maxResult > self.threshold:
                passes = False
            return {'pass': passes, 'data': maxResult, 'threshold': self.threshold}
        else:
            return {'pass': True, 'data': 0.0, 'threshold': self.threshold}
