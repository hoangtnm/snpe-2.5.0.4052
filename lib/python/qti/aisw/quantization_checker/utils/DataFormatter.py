...
from qti.aisw.quantization_checker.utils.utils import makeSubdir
...
class DataFormatter:
    def __init__(self, outputDir, inputModel, inputList, quantizationVariations, logger=Logger('qnn-quantization-checker-log')):
        ...
        self.weightResults = {}
        self.biasResults = {}
        self.inputAnalysisCsvHeader = 'Input Filename, Passes Verification, Verification Results, Threshold for Verification, Verifier Name'
        self.weightsAndBiasesAnalysisCsvHeader = 'Node Name, Tensor Name, Passes Verification, Verification Results, Threhsold for Verification, Verifier Name'
        self.activationsAnalysisCsvHeader = self.weightsAndBiasesAnalysisCsvHeader + ', Input Filename'
        self.htmlHeadBegin = '''
            <html>
    ...
    def __formatActivationsForCsv(self, quantizationVariation):
        quantOptionResults = []
        inputFileResults = self.activationResults[quantizationVariation]
        for activationNodeName, inputFileResult in inputFileResults.items():
            for inputFilename, comparisonResults in inputFileResult.items():
                for comparisonAlgorithmName, comparisonResult in comparisonResults.items():
                    if comparisonResult != dict():
                        ...
                        # [opName, tensorName, perAlgorithmResult[1][0], perAlgorithmResult[1][1], perAlgorithmResult[1][2], perAlgorithmResult[0]]
                        quantOptionResults.append([activationNodeName[0], activationNodeName[1], comparisonResult['pass'], comparisonResult['data'], comparisonResult['threshold'], comparisonAlgorithmName, inputFilename])
        return quantOptionResults

    def __formatInputsForCsv(self):
        inputAnalysisResults = []
        for filename in self.inputResults:
            for comparisonAlgorithmName, comparisonResult in self.inputResults[filename].items():
                if comparisonResult != dict():
                    inputAnalysisResults.append([filename, comparisonResult['pass'], comparisonResult['data'], comparisonResult['threshold'], comparisonAlgorithmName])
        return inputAnalysisResults

    def __formatWeightsForCsv(self, quantizationVariation):
        quantOptionResults = []
        for opAndNodeName, comparisonResults in self.weightResults[quantizationVariation].items():
            for comparisonAlgorithmName, comparisonResult in comparisonResults.items():
                if comparisonResult != dict():
    ...
    def __formatBiasesForCsv(self, quantizationVariation):
        quantOptionResults = []
        for opAndNodeName, comparisonResults in self.biasResults[quantizationVariation].items():
            for comparisonAlgorithmName, comparisonResult in comparisonResults.items():
                if comparisonResult != dict():
    ...
    def printCsv(self):
        ...
        np.savetxt(os.path.join(csvOutputDir, 'input_data_analysis.csv'), self.__formatInputsForCsv(), delimiter=',', fmt='%s', comments='', header=self.inputAnalysisCsvHeader)
        for quantizationVariation in self.quantizationVariations:
            if quantizationVariation != 'unquantized' and quantizationVariation in self.activationResults:
                np.savetxt(os.path.join(csvOutputDir, quantizationVariation + '_activations.csv'), self.__formatActivationsForCsv(quantizationVariation), delimiter=',', fmt='%s', comments='', header=self.activationsAnalysisCsvHeader)
            if quantizationVariation in self.weightResults:
                np.savetxt(os.path.join(csvOutputDir, quantizationVariation + '_weights.csv'), self.__formatWeightsForCsv(quantizationVariation), delimiter=',', fmt='%s', comments='', header=self.weightsAndBiasesAnalysisCsvHeader)
            if quantizationVariation in self.biasResults:
                np.savetxt(os.path.join(csvOutputDir, quantizationVariation + '_biases.csv'), self.__formatBiasesForCsv(quantizationVariation), delimiter=',', fmt='%s', comments='', header=self.weightsAndBiasesAnalysisCsvHeader)
