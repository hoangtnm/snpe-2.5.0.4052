...
class QOptionGenerator:
    def __init__(self, quantizationVariations, modelFile, caffeBin, inputList, sdkDir, activationWidth, biasWidth, weightWidth, outputDir, quantOverridesPath, config_file, logger):
        self.quantizationVariations = quantizationVariations
        self.quantAlgorithms = []
        self.quantOptions = []
        self.modelFile = modelFile
        self.caffeBin = caffeBin
    ...
    def generate(self):
        ...
        if self.modelFile.endswith(".pb"):
            ...
            mlFramework = em.ONNX
        elif self.modelFile.endswith(".prototxt"):
            mlFramework = em.CAFFE
        else:
        ...
        if mlFramework == em.TENSORFLOW:
            ...
            converter = TensorflowConverter(self.logger, self.sdkDir, self.modelFile, self.inputList, self.quantOptions, self.quantAlgorithms)
        elif mlFramework == em.TFLITE:
            from qti.aisw.quantization_checker.utils.ConverterTools import TfliteConverter
            converter = TfliteConverter(self.logger, self.sdkDir, self.modelFile, self.inputList, self.quantOptions, self.quantAlgorithms)
        ...
        elif mlFramework == em.CAFFE:
            if not self.caffeBin or not self.caffeBin.endswith(".caffemodel"):
                self.logger.print("ERROR! Trying to convert Caffe Models, please use --model_file model.protoxt --caffe_bin model_bin.caffemodel as parameters.")
                return -1
            from qti.aisw.quantization_checker.utils.ConverterTools import CaffeConverter
            converter = CaffeConverter(self.logger, self.sdkDir, self.modelFile, self.caffeBin, self.inputList, self.quantOptions, self.quantAlgorithms)
        else:
            self.logger.print("ERROR! Input model_file not recognizeable. Please use a model file with a .pb .onnx or .prototxt extension.")
            return -1
        try:
