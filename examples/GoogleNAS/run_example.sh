...
cd $USER_HIL_DATA
python $SDK_ROOT/examples/GoogleNAS/mnist_example_data.py
...
echo "$SDK_ROOT/examples/GoogleNAS/build.sh -c $CERT_LOCATION -n $NAS_CODE_LOCATION -p $PROJECT -d qc_hil -r $REGION"
$SDK_ROOT/examples/GoogleNAS/build.sh -c $CERT_LOCATION -n $NAS_CODE_LOCATION -p $PROJECT -d qc_hil -r $REGION
