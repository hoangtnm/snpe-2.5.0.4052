...
def setup_assets(alexnet_data_dir, download, runtime, enable_htp):
    # print('Setup alexnet...')
    ...
    if ('dsp' == runtime or 'aip' == runtime or 'all' == runtime):
        cmd = ['snpe-dlc-quantize',
               '--input_dlc', os.path.join(dlc_dir, 'bvlc_alexnet.dlc'),
               '--input_list', os.path.join(data_dir, 'cropped/raw_list.txt'),
               '--output_dlc', os.path.join(dlc_dir, 'bvlc_alexnet_quantized.dlc')]
        if (enable_htp):
